...
from kip.domain.starter_archive import StarterArchiveReceipt
...
from kip.starter_archive import (
    ...
    default_starter_archive_output,
...
app = typer.Typer(
    ...
    help="Build and verify the minimal online KIP source starter ZIP",
)
...
def build_command(
    ...
    try:
        resolved_output = output or default_starter_archive_output(resolved_root)
...
def verify_command(
    ...
    try:
        receipt = verify_starter_archive(archive)
    except (KipError, OSError, PydanticValidationError) as error:
        _emit_error(error)
        raise typer.Exit(code=3) from error
    _emit_receipt(receipt)
...
def _emit_receipt(receipt: StarterArchiveReceipt) -> None:
    envelope = Envelope(
        ok=True,
        data=receipt.model_dump(mode="json"),
        meta=EnvelopeMeta(request_id=new_id("req"), workspace="starter-kit"),
    )
