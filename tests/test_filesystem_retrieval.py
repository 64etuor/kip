...
def test_sync_search_context_and_stale_detection(test_container):
    ...
    hits = test_container.service.search(context, SearchRequest(query="참여율 변경 승인", limit=10))
    assert hits
    assert "참여율" in hits[0].snippet

    bundle = test_container.service.context_bundle(
        context,
    ...
    stale = test_container.service.read_unit(context, unit_id)
