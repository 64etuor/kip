...
"$(python_cmd)" - "$BASE_URL" "$EMBED_SERVED" "$RERANK_SERVED" <<'PY'
import sys

import httpx

base_url, embedding_model, reranker_model = sys.argv[1:]
with httpx.Client(timeout=120) as client:
    ...
    if len(vector) != 1024:
        raise SystemExit(f"expected 1024 embedding dimensions, got {len(vector)}")
    rerank = client.post(
        f"{base_url}/rerank",
        json={
...
print("semantic smoke passed: embedding=1024, reranker_top=1")
PY
