...
자연어 에이전트 품질은 도구 단위 테스트와 구분해 실제 MCP 호출·근거 재열람·
최종 주장으로 검증한다. 현재 범위와 미검증 항목은
[`AGENT_QUALITY.md`](AGENT_QUALITY.md)에 기록한다. 얕은 워크북이나 불완전한
CSV의 답변 사용 가능 여부는 특정 언어·단어가 아닌 근거의 완전성으로
판단한다(ADR-058).

Agent 지침은 작업별 문서 경로와 코드·도구 스키마만으로 알기 어려운 운영
주의점에 집중한다(ADR-055). 전체 설계 문서를 매번 읽거나 CLI 인자 목록을
Skill에 복제하지 않는다. 근거 재열람, 원본 불변성, ACL, 명시적 검토 결정은
짧은 진입점에도 유지한다. 명시한 KIP 경로가 잘못되면 다른 workspace를
대신 검색하지 않고 실패해야 한다.
