...
- **Last reviewed:** 2026-09-20 (3.16.0 layering and adapter parity; 3.15.8 documentation and gate release; 3.15.7 comma-ACL last line, `semantic_disabled`, setup sample preset, MCP doctor; retrieval numbers remain the 3.12.0 evidence)
...
| Application boundaries | Focused use cases depend on capability ports; concrete adapters are selected only at composition | `Application` composes ingestion, retrieval, evidence, answer, knowledge, operations, telemetry, ontology, interaction, and diagnostics use cases. Since 3.16.0 `kip.architecture_rules` defines an allow-list per inner layer and an edge rule, consumed by the gate and the boundary tests; use cases receive frozen configuration records and reach the ontology tree through ports. Remaining gaps (2026-09-20 audit): `PostgresDatabase` is one 4,279-line class that the PostgreSQL capability stores forward to while the memory family is genuinely decomposed; `RetrievalStore.search` carries a PostgreSQL `lexemes` argument; the core `StrictModel` is mutable and identifiers are untyped strings | Aligned at the layer boundary; adapter-side cohesion remains |
