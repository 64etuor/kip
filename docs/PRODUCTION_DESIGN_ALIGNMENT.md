...
| Agent-guided setup | One-question-at-a-time inspect/plan/apply/verify flow writes approved local artifacts atomically and emits a redacted receipt | The setup state machine, safe-root checks, secret references, atomic apply, and receipt contracts are implemented. A 2026-08-10 ephemeral no-context acceptance passed | Aligned for starter acceptance; each recipient must rerun with its real mounts, identity, backup, and egress decisions |
| Runtime topology | Digest-pinned non-root API, worker, migration, and PostgreSQL services; sources read-only; identity/TLS/secrets supplied by the deployment | `compose.production.yaml`, role templates, resource limits, read-only containers, private database network, and loopback API binding exist. It is a reference Compose deployment, not an orchestrator, TLS edge, or secret manager | Partial: hardened reference ready, environment control plane remains external |
| Connector coverage | Filesystem/NAS, Slack, and mail preserve stable IDs, revisions, cursors, tombstones, ACL snapshots, and outage safety | Filesystem is pilot-validated. Slack, Apple Mail, and IMAP are reference adapters requiring provider-specific scope, edit/delete, rate-limit, retention, permission, and UID validation | Partial by design |
| Parser, OCR, and exact evidence | Parsers emit reproducible locators through shadow activation; OCR is optional; spreadsheet facts use exact original ranges | PDF and native HWP/HWPX retrieval paths are pilot-validated, with HWP locator granularity still incomplete. OCR is routed but not bundled. XLSX original-range results serialize date, datetime, and time values through the versioned JSON edge | Partial: HWP locator and bundled OCR gaps remain; XLSX date edge aligned |
...
| Review surface | Production design is headless; review is available through stable machine-readable edges | CLI/API review workflows exist; there is no bundled web review UI | Aligned; earlier README wording was stale |
...
| Ontology RAG and graph | Only reviewed assertions enter current graph context; every path reopens exact, ACL-visible evidence; Neo4j is optional | Typed candidates, mining/review, migration lineage, approved PostgreSQL graph context, exact evidence, and evaluation contracts exist. Neo4j remains a port, and broad private ontology quality has not been established | Ready for pilot, not broad ontology-RAG acceptance |
| Observability | Vendor-neutral redacted traces/metrics expose pipeline decisions without creating a second sensitive corpus | Closed PostgreSQL query traces, admin inspection/pruning, and optional OTLP spans/metrics are implemented. Collector delivery is non-blocking; dashboards, alerts, schedules, and collector credentials are deployment responsibilities | Aligned for pilot; operational alert coverage is environment-specific |
| Backup and recovery | PostgreSQL, CAS, configuration, migrations, ACL/RLS, and rebuildable projections restore into an empty target with fingerprints | Sealed backup/restore tooling and a checksummed restore drill are implemented; the 2026-08-10 acceptance restored canonical data and rebuilt lexical projection. Encryption, off-host retention, scheduling, and alerting are external | Ready for operational adoption, not a managed backup service |
...
| ACL negative testing | Unauthorized principals and scopes reveal neither content nor existence | Repository/RLS probes reported zero leaks. Explicit local CLI scope options replace ambient grants, and an explicit empty set remains empty | Aligned at repository and local edge boundaries |
...
| Wide XLSX range with dates | Date/datetime/time values serialize as ISO 8601 JSON scalars | Pass at contract boundary |
...
6. Complete HWP locator, OCR, restore, connector/provider, pagination, and
   recipient-environment acceptance evidence.
