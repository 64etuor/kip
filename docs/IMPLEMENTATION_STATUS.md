...
## 2026-09-16 comma ACL last line and agent retrieval CX (3.16.0, unreleased)
...
scopes, evaluation datasets, `RequestContext`, and the PostgreSQL session GUC
immediately before `set_config('kip.acl_scopes', ...)`. The sibling
`kip.roles` GUC is fail-closed the same way. `workspace:`, `group:` and
`project:` scopes without a comma keep working. Search reports
`semantic_disabled` when semantic search is off. `kip setup preset sample`
fills remaining questions from bundled `sample-data`. MCP `kip_doctor` shares
the CLI payload, including English `summary_en`. Context packs keep head and
tail of a long unit (`body_truncated` is not a leading prefix). XLSX shallow
chunks overlap with true span locators. Python `kip` / `kip-mcp` refuse a
loopback URL on another deployment's port. A Korean+English query splitter is
tested but not wired into default lexical ranking: a trial injection dropped
the private lexical floor.

This does not replace the 3.12.0 retrieval numbers, expand the 19-case private
golden set, re-run the 3.6.1 agent-quality harness, or swap the embedding
identity. Those remain the next quality gates in
...
fused ranking with `rerank_degraded` when only the reranker fails; and
`lexical_rerank_degraded` marks a lexical reranker that failed or is not
configured in the semantic fallback. `docs/DATA_CONTRACTS.md` is the canonical
list of these codes. They are reported in search/context `meta.warnings` on
every edge. Explicit vector-family modes still fail. A per-adapter circuit skips a failed runtime for 30 s; query embeddings use a
10 s timeout with a 3 s connect timeout.

Lexical performance: query n-grams present in at least
`max(200, total_units * search.lexical_common_term_fraction)` lexical units
(a shared folder name such as `절차` had matched 55k of 104k units) are left
out of candidate matching while BM25 still scores the full question. With the
default fraction of 0.02 the 200-unit floor governs every corpus below 10,000
units, and the probe is skipped below about 400 units (twice the cutoff)
because a corpus that small has no meaningfully common term; the unit total is
...
| PostgreSQL migrations | Ready as the production reference profile | Workspace, required-scope, ACL-snapshot freshness, and owner-bound `FORCE ROW LEVEL SECURITY` on every RLS-enabled table are included. Through 3.12.2 only 16 of 34 tables forced it — migration 0016 named six core tables and the other 18 stayed ENABLE-only, which is inert for the table owner — so an owner-connected API, worker, or maintenance session read and wrote them with no workspace or ACL predicate. Migration 0028 forces it schema-wide in 3.13.0 and an integration test asserts that `relrowsecurity` implies `relforcerowsecurity`. Normal migration installs pgvector, the 1024d projection, and migration 0018's cosine HNSW index; since 3.12.0 a complete release-reviewed semantic space activates automatically, other identities need explicit activation |
...
| Query tracing and metrics | Ready for pilot | PostgreSQL/RLS canonical redacted traces, admin-only CLI/REST inspection, bounded retention pruning, and non-fatal delivery. The OpenTelemetry exporter adapter was removed in 3.13.0; `QueryTraceExporter` remains a port with no adapter behind it, so nothing is exported and a deployment that wants an exporter supplies one through `build_container` |
