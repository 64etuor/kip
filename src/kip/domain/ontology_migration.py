"""Ontology migration manifests and the diff they must cover.

A migration is a decision about meaning, so the manifest, the diff it answers
and the coverage rules are domain data. Reading either ontology tree from disk
is :mod:`kip.ontology_migration`, reached through
:class:`kip.ports.ontology.OntologyCatalogPort`.
"""
...
from kip.domain.ontology import OntologyModel, OntologyRelease
...
def diff_releases(before: OntologyRelease, after: OntologyRelease) -> OntologyDiff:
    """Classify every entity and predicate change between two releases."""
